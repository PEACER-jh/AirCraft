...
ClassifyNode::ClassifyNode(const rclcpp::NodeOptions & options) : rclcpp::Node("classify_node", options)
{
    ...
    // this->image_mark_pub_ = this->create_publisher<sensor_msgs::msg::Image>("/image/mark", 10);
    this->image_debug_pub_ = this->create_publisher<sensor_msgs::msg::Image>("/image/debug", 10);

    image_sub_ = std::make_shared<image_transport::Subscriber>(image_transport::create_subscription(
                this, "/image/raw", std::bind(&ClassifyNode::ImageCallBack, this, std::placeholders::_1),
...
void ClassifyNode::ImageCallBack(const sensor_msgs::msg::Image::ConstSharedPtr & img)
{
    ...
    cv::imshow("image", this->image_);
    cv::imshow("edges", edges);

    this->image_debug_pub_->publish(*cv_bridge::CvImage(img->header, "bgr8", edges).toImageMsg());
}
